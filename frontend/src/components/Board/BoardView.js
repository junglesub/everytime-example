...
function BoardView({ history, match }) {
  const userFrom = parseInt(localStorage.getItem("userId"));
  ...
  const FetchBoard = () => {
    axiosInstance
      .get(`${endpoints.board.list}?page=${currentPage}`)
      .then((response) => {
  ...
  const onSubmit = (e) => {
    ...
    let variables = {
      ...
      isAnonymous: WriterIcon,
    };
    axiosInstance.post(endpoints.board.create, variables).then((response) => {
      if (response.status === 200) {
        setInput({
          boardTitle: "",
          boardContent: "",
            ...
            return (
              <React.Fragment key={index}>
                <AddBoard
                  id={board.postId}
                  user={board.memberId}
                  time={board.createdDate}
                  writer={board.anonymous ? "익명" : board.nickname}
                  title={board.title}
