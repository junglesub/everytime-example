...
function BoardDetail(props) {
  ...
  useEffect(() => {
    const variable = { boardId: BoardId }; // TODO :
    axiosInstance.get(`${props.match.path}/${BoardId}`).then((response) => {
  ...
  const FetchComment = () => {
    axiosInstance.get(endpoints.comment.list, variables).then((response) => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    axiosInstance.post(endpoints.comment.create, variables).then(() => {
      alert("댓글이 등록되었습니다.");
      setValue("");
      FetchComment();
    });
